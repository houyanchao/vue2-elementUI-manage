<template>
  <div class="dispose-image" :style="viweStyle" @click="onClick">
    ...
    <img v-if="status == STATUS.DID_LOAD" v-bind:src="imgUrl" v-bind:style="imageStyle" />
    <slot></slot>
  </div>
</template>
...
export default {
  ...
  watch: {
    src(val, oldVal) {
      if (val != oldVal && !this.isLazy) {
        ...
        let imgUrl = upyunConfig.domain+val
  ...
  computed: {
    imgUrl(){
      return upyunConfig.domain+this.src
    },
    viweStyle() {
      return {
        position: 'relative',
