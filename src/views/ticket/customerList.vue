    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-button size="mini" @click="courseStatistic(scope.row)">参课统计</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
  ...
  export default {
    ...
    methods: {
      ...
      courseStatistic(row){
        let organization_id = encodeURIComponent(row.organization_id)
        let organization_name = encodeURIComponent(row.organization_name)
        this.$router.push({path: '/ticket/course-statistic', query: { organization_id: organization_id, organization_name: organization_name}});
      },
      exportExcel(){
