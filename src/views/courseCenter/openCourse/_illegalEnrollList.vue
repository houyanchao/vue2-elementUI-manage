<template lang="html">
  <div class="wx-illegal-enroll">
    <el-dialog title="后台报名人员" :center="true" :visible="isShowIllegalEnroll" @close="close" :width="'80%'">
      <el-table :max-height="500" :data="datas" :stripe="true" :empty-text="'暂无数据'" border>
        ...
        <el-table-column  prop="operation_user" label="报名业务员"></el-table-column>
        <el-table-column  label="报名费用">
          <template slot-scope="scope">
            <span>{{parseInt(scope.row.price) / 100}}</span>
          </template>
        </el-table-column>
        <el-table-column  prop="enroll_time" label="报名时间"></el-table-column>
        ...
        <el-table-column label="操作" fixed="right" width="320">
          <template slot-scope="scope">
            ...
            <el-button v-if="scope.row.can_cancel == 'Y'"
              size="small" @click="cancel(scope.$index, scope.row)">
              取消报名
            </el-button>
          </template>
        </el-table-column>
    ...
    <send-message v-if="isShowCourseInfo" :isShowCourseInfo="isShowCourseInfo"
      :rowInfo="rowInfo" :content="content" @close="closeMessage">
    </send-message>
  </div>
</template>
