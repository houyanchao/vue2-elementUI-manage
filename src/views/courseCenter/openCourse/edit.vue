  ...
  <div class="wx-open-course-edit">
    <el-form ref="courseInfo"  :model="courseInfo" label-width="120px">
      ...
      <el-form-item label="招生人数" required>
        <el-input v-model="courseInfo.maxCount" @blur="changMaxCount" placeholder="请输入课程招生名额上限" class="width_400"></el-input>
      </el-form-item>
      ...
      <el-form-item>
        <el-button v-if="status == '-1'" type="primary" plain @click="submitForm('courseInfo', true)">保存计划开课</el-button>
        <el-button type="primary" @click="submitForm('courseInfo', false)">立即 提交</el-button>
      </el-form-item>
    </el-form>
...
export default {
  ...
  data () {
    return {
      ...
      isShowMap: false,
      isWatchTeacher: 0,  // 当其大于1时开启监听模式
      status: '',  // 当前课程他状态
      testDate: '',
      currentAction: 'create',  //  create：创建 reopen：重开 edit：编辑
      openCourseId: '',
      isReopen: '',  // 是否重开课程 0:非重开课程 1：重开课程
      categoryFirst: [],     // 一级分类
      categorySecond: [],    // 二级分类
      provinces: [],         // 省份数组
      citys: [],             // 市数组
      ...
      courseInfo: {
        ...
        enrollDate: [],  //  报名时间
        isGetCourseInfo: false,  // 首次进入编辑页不改变数据 后期如果把新建加上则进入页面就改为true
        location: {
          province_id: '',
          city_id: '',
          area_id: ''
  ...
  watch: {
    teachers(newVal, oldVal) {
      if (this.isWatchTeacher > 1) {
        this.courses = []
        this.initCourseInfo()
      }
    },
    courses: function(newVal, oldVal) { // 课程数组永远只有一项但是不排除会不会加临时课程
      let that = this
      if (this.isGetCourseInfo) {
        newVal.forEach(item => {
          that.getCourseInfo(item.course_source_id)
        })
      }
      this.isGetCourseInfo = true
    },
  ...
  methods: {
    ...
    changMaxCount() {
      if (this.courseInfo.maxCount < this.courseInfo.enrollNum) {
        this.$message('不可小于已报名人员总数')
        this.courseInfo.maxCount = this.courseInfo.enrollNum
      }
    },
    checkNumber() {
    ...
    getQuery() {
      this.openCourseId = this.$route.query.courseId
      this.isReopen = this.$route.query.isReopen || 0
    },
    getCourseInfo(id) {
    ...
    changeCurrentAction() {
      if (this.openCourseId) {
        this.currentAction = (this.isReopen == 1) ? 'reopen' : 'edit'
      }
    },
    initData(data) {
      this.isWatchTeacher++  // 当其大于1时开启监听 第一次不监听
      ...
      this.courses.push({
        title: data.title,
        course_source_id: data.course_source_id
      })
      course.courseSourceId = data.course_source_id || ''  // 没实际意义
      this.initCourseInfo(data)
      course.isCancel = data.can_cancel
      if (this.currentAction != 'reopen') {
        // 开课时间  课程表
        course.syllabus = data.time
        course.startCourseDate = [course.syllabus[0].date, course.syllabus[course.syllabus.length - 1].date]
        course.isShowSyllabus = course.syllabus.length > 0 ? true : false
        // 报名时间
        course.enrollDate = [__tool.initDateTime(data.apply_time_start), __tool.initDateTime(data.apply_time_end)]
        if (course.isCancel == 'N') {
          course.cancelDate = ''
        }
        else {
          course.cancelDate = __tool.initDateTime(data.cancel_time)
        }
      }

      course.location.province_id = data.location.province_id
      course.location.city_id = data.location.city_id
      course.location.area_id = data.location.area_id
      this.provinceChange(course.location.province_id, true)
      this.cityChange(course.location.city_id, true)
      course.address = data.address
      course.userLimit = data.limit_count
      course.maxCount = data.max_count
      course.ticketCount = data.ticket_count
      course.originalPrice = data.original_price
      course.price = data.price
      course.hotline = data.hotline
      course.enrollNum = parseInt(data.enroll_num) > 0 ? parseInt(data.enroll_num) : 0
      this.isDisabled = course.enrollNum > 0 ? true : false
      this.changMap()
    },
    initCourseInfo(obj) {
      let data = obj || ''
      ...
      if (data) {
        ...
        // 课程主图  缩略图
        course.initMasterImages = !data.image ? [] : [{url: data.image}]
        course.masterImage = data.image
        course.initBreviaryImages = !data.thumb ? [] : [{url: data.thumb}]
    ...
    getOpenCourseInfo() {
      openCourseApi.getOpenCourseDetail(
        {
    ...
    submitForm(form, isPlan){
      ...
      if(!course.originalPrice || parseInt(course.originalPrice) < 0) {
        this.$message.error('报名费用不能为空或小于0')
        return
      }

      if(parseInt(course.price) < 0 || parseInt(course.price) > parseInt(course.originalPrice)) {
        this.$message.error('优惠费用不能小于0或大于报名费用')
        return
      }

      if (course.price && parseInt(course.price) < 0) {
      ...
      if (isPlan) {
        ...
        openCourseApi.updateOpenCourse(
          {
            'title': course.title,//	字符串	必填			课程标题
            'course_id': this.openCourseId,
            'course_source_id': courseSourceId,
